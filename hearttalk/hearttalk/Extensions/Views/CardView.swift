...
struct CardView: View {
    ...
    private func makeLikeButton(_ card: Card) -> some View {
        ...
        .contextMenu {
            ForEach(viewModel.myPacks, id: \.self) { pack in
                if !pack.cards.contains(card) {
                    Button {
                        HapticManager.shared.triggerHapticFeedback(.light)
                        SoundManager.shared.sound(.click1)
                    ...
                    Button {
                        HapticManager.shared.triggerHapticFeedback(.light)
                        SoundManager.shared.sound(.click1)
                        viewModel.removeCard(card, from: pack)
                    } label: {
    ...
    private func makeNotesButton() -> some View {
        Button {
            ...
            Image("notes")
                ...
                .resizable()
                .foregroundStyle(.lightBlack)
                .frame(width: UIDevice.current.userInterfaceIdiom == .phone ? 16 : 40, height: UIDevice.current.userInterfaceIdiom == .phone ? 16 : 40)
        }
    }
